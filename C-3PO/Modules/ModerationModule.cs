...
namespace C_3PO.Modules
{
    public class ModerationModule : AppInteractionModuleBase
    {
            ...
            [Summary("reason", "The reason behind the warning.")] string reason)
        {
            ...
            await _logsService.Log($"{user.Mention} has been warned by {Context.User.Mention}.\n\n```{reason}```");
        }
        ...
        public async Task RemoveWarning([Summary("id", "The ID of the warning.")] int id)
        {
            ...
            await _logsService.Log($"Warning {id} (from <@{warning.User}>) was removed by {Context.User.Mention}.");
        }
        ...
        public async Task Purge([Summary("amount", "The amount of messages to be purged.")] int amount)
        {
            ...
            await _logsService.Log($"{messages.Count()} messages were purged by {Context.User.Mention} in <#{Context.Channel.Id}>.");
        }
            ...
            [Summary("reason", "The reason behind the kick.")] string? reason = null)
        {
            ...
            var ejected = Context.Guild.GetRole(_configuration.Roles.Ejected);
            ...
            await _logsService.Log($"{user.Mention} was kicked by {Context.User.Mention}.{(string.IsNullOrEmpty(reason) ? "" : $"\n\n```{reason}```")}");
        }
            ...
            [Summary("expiresOn", "The date and time when the ban expires.")] string? expiresOnString = null)
        {
            ...
            try
            {
                await user.SendMessageAsync($"Hello there! You have been banned {(expiresOn == default(DateTime) ? "permanently" : $"until {expiresOn:g}")} from Efehan's Hangout by {Context.User.Mention}. You're now floating in space...{(string.IsNullOrEmpty(reason) ? "" : $"\n\n```{reason}```")}");
            }
            catch
            { }

            var duration = (expiresOn - DateTime.Now).Humanize(3, minUnit: TimeUnit.Minute);

            await UpdateAsync($"Been banned from Efehan's Hangout, {user.Mention} has.");
            await _logsService.Log($"{user.Mention} was banned {(expiresOn == default(DateTime) ? "permanently" : $"for {duration}")} " +
                $"by {Context.User.Mention}.{(string.IsNullOrEmpty(reason) ? "" : $"\n\n```{reason}```")}");
        }
        ...
        public async Task Unban([Summary("user", "The user to unban.")] SocketGuildUser user)
        {
            ...
            var ejected = Context.Guild.Roles.First(x => x.Id == _configuration.Roles.Ejected);
            ...
            await _logsService.Log($"{user.Mention} was unbanned by {Context.User.Mention}.");
        ...
        public async Task Lockdown([Summary("enable", "Whether or not to enable lockdown mode.")] [Choice("true", "true"), Choice("false", "false")] string input)
        {
            ...
            if ((UserJoinedHandler.Lockdown && shouldEnable) || (!UserJoinedHandler.Lockdown && !shouldEnable))
            {
                await UpdateAsync($"The ship is {(shouldEnable ? "already" : "not")} in lockdown mode.");
                return;
            }

            UserJoinedHandler.Lockdown = shouldEnable;
            await DbContext.SaveChangesAsync();
            ...
            await _logsService.Log($"Lockdown mode has been {(shouldEnable ? "enabled" : "disabled")} by {Context.User.Mention}.");

            if (shouldEnable)
                return;

            var ejected = Context.Guild.Roles.First(x => x.Id == _configuration.Roles.Ejected);
            var unidentified = Context.Guild.Roles.First(x => x.Id == _configuration.Roles.Unidentified);
